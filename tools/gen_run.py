...
REPLAY_SRC = REPO / "renode" / "replay.py"
...
REPLAY_PLATFORM_TEMPLATE = """// GENERATED by tools/gen_run.py - do not edit.
// Replay platform. There is no seed anywhere in this file or the scripts it
// references: the peripherals serve values taken from a trace, and nothing else.
...
def render_replay(values: list[int], exhausted_log: pathlib.Path) -> str:
    """Bake a value stream into a copy of the replay peripheral.

    Deliberately takes values, not a seed. There is no parameter here that could
    carry one, which is the point: replay reconstructs a run from evidence or it
    does not reconstruct it at all.
    """
    src = REPLAY_SRC.read_text(encoding="utf-8")
    # A plain line check rather than a regex: this guard is the thing
    # standing between "reproduced from evidence" and "re-ran the
    # experiment", and it should be readable without parsing escapes.
    if any(line.startswith("SEED") for line in src.splitlines()):
        raise SystemExit(
            f"{REPLAY_SRC}: contains a SEED line. The replay peripheral must not "
            "be able to reach the seed, or reproduction proves nothing."
        )
    for pattern, replacement in (
        (r"^VALUES = .*$", "VALUES = [" + ",".join(str(v) for v in values) + "]"),
        (r"^EXHAUSTED_LOG = .*$",
         "EXHAUSTED_LOG = " + repr(exhausted_log.resolve().as_posix())),
    ):
        src, count = re.subn(pattern, replacement, src, count=1, flags=re.MULTILINE)
        if count != 1:
            raise SystemExit(
                f"{REPLAY_SRC}: expected one line matching {pattern!r}, found {count}"
            )
    _require_ascii(REPLAY_SRC, src)
    return src


def generate_replay_resc(
    tag: str,
    out_dir: pathlib.Path,
    uart_out: pathlib.Path,
    sensor_values: list[int],
    jitter_values: list[int],
    exhausted_log: pathlib.Path,
    run_for: str = "0.5",
    elf: pathlib.Path | None = None,
) -> pathlib.Path:
    """Emit a run script that replays a recorded value stream."""
    out_dir = out_dir.resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    sensor_path = out_dir / f"replay_sensor_{tag}.py"
    sensor_path.write_text(render_replay(sensor_values, exhausted_log), encoding="utf-8")
    jitter_path = out_dir / f"replay_jitter_{tag}.py"
    jitter_path.write_text(render_replay(jitter_values, exhausted_log), encoding="utf-8")
    ...
    resc_path = out_dir / f"replay_{tag}.resc"
    resc_path.write_text(
        RESC_TEMPLATE.format(
            seed=tag,
            platform=platform_path.as_posix(),
